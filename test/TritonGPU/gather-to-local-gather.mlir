...
module attributes {"ttg.target" = "cuda:90", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK-LABEL: @int4_gather
  // CHECK: %[[BYTES:.*]] = arith.extui %arg0 : tensor<256xi4, #{{.*}}> to tensor<256xi8, #{{.*}}>
  // CHECK-NEXT: %[[BUF:.*]] = ttg.local_alloc %[[BYTES]]
  // CHECK-SAME: !ttg.memdesc<256xi8, #{{.*}}, #{{.*}}>
  // CHECK-NEXT: %[[RESULT:.*]] = ttg.local_gather %[[BUF]][%arg1]
  // CHECK-NEXT: %[[INT4:.*]] = arith.trunci %[[RESULT]] : tensor<64xi8, #{{.*}}> to tensor<64xi4, #{{.*}}>
  // CHECK-NEXT: tt.return %[[INT4]]
  tt.func @int4_gather(%src: tensor<256xi4, #blocked>, %idx: tensor<64xi32, #blocked>) -> tensor<64xi4, #blocked> {
    %result = tt.gather %src[%idx] {axis = 0 : i32} : (tensor<256xi4, #blocked>, tensor<64xi32, #blocked>) -> tensor<64xi4, #blocked>
    tt.return %result : tensor<64xi4, #blocked>
  }
