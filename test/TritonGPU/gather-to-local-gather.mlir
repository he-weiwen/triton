// RUN: triton-opt %s -split-input-file -verify-diagnostics -tritongpu-gather-to-local-gather="hoist=true" -cse | FileCheck %s --check-prefixes=CHECK,HOIST --implicit-check-not=ttg.local_alloc
// RUN: triton-opt %s -split-input-file -verify-diagnostics -tritongpu-gather-to-local-gather -cse | FileCheck %s --check-prefixes=CHECK,NOHOIST --implicit-check-not=ttg.local_alloc
...
module attributes {"ttg.target" = "cuda:90", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK: %[[INTS:.*]] = tt.ptr_to_int %arg0
  // CHECK-NEXT: %[[BUF:.*]] = ttg.local_alloc %[[INTS]]
  // CHECK-SAME: !ttg.memdesc<256xi64, #{{.*}}, #{{.*}}>
  // CHECK-NEXT: %[[RESULT:.*]] = ttg.local_gather %[[BUF]][%arg1]
  // CHECK-NEXT: %[[PTRS:.*]] = tt.int_to_ptr %[[RESULT]]
  // CHECK-NEXT: tt.return %[[PTRS]]
  ...
  // CHECK-LABEL: @gather_loop_invariant_source
  // HOIST: %[[BUF:.*]] = ttg.local_alloc %arg0
  // CHECK: scf.for
  // NOHOIST: %[[BUF:.*]] = ttg.local_alloc %arg0
  // CHECK: %[[NEXT:.*]] = ttg.local_gather %[[BUF]]
  // CHECK-NEXT: scf.yield %[[NEXT]]
  tt.func @gather_loop_invariant_source(%src: tensor<256xi32, #blocked>, %idx: tensor<256xi32, #blocked>, %n: i32) -> tensor<256xi32, #blocked> {
    %c0 = arith.constant 0 : i32
    %c1 = arith.constant 1 : i32
    %result = scf.for %i = %c0 to %n step %c1 iter_args(%current = %idx) -> tensor<256xi32, #blocked> : i32 {
      %next = tt.gather %src[%current] {axis = 0 : i32} : (tensor<256xi32, #blocked>, tensor<256xi32, #blocked>) -> tensor<256xi32, #blocked>
      scf.yield %next : tensor<256xi32, #blocked>
    }
