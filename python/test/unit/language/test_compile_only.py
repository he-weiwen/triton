...
# Triton merges the two shared-memory copies. NVIDIA Triton also hoists the
# copy out of the loop. Gluon keeps a separate copy for each gather.
@pytest.mark.parametrize(
...
def test_compile_only_gather_shared_source(frontend, target, expected_allocations, hoist_source):
    ...
    if hoist_source:
        ...
        assert "ttg.local_alloc" not in before_loop
        assert loop_body.count("ttg.local_alloc") == expected_allocations
